...
+ (CGFloat)hyb_heightForTableView:(UITableView *)tableView
    ...
    if (cache) {
        ...
        NSIndexPath *key = cacheKeys[kHYBCacheUniqueKey];
...
- (CGFloat)getCellHeight {
    [self setNeedsUpdateConstraints];//设置重新更新约束标记
    [self updateConstraintsIfNeeded];//在当前runloop中立即触发约束更新，自动更新布局。
    ...
    if (self.hyb_lastViewInCell) {
        ...
        NSArray *views = self.hyb_lastViewsInCell;
        if (views.count == 0){
            views = self.contentView.subviews;
        }
        for (UIView *bottomView in views) {
...
- (CGFloat)private_hyb_heightForTableView:(UITableView *)tableView {
    CGFloat rowHeight = [self getCellHeight];
